  ...
  return (
    <div className="mb-5 flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-extrabold tracking-tight text-slate-950 md:text-2xl">{title}</h2>
      </div>
      {linkTo ? (
        <Link
          to={linkTo}
      ...
      <div className="bg-white font-sans text-slate-900">
        <div className="mx-auto max-w-7xl px-4 py-6 md:px-8">
            ...
            <div className="grid items-center gap-10 md:grid-cols-2">
              <div className="relative z-10">
                <span className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-white px-4 py-2 text-sm font-bold text-orange-600 shadow-sm">
                  <Icon size={15}>{icons.flame}</Icon> New Arrivals
                </span>
                <h1 className="mt-6 max-w-xl text-5xl font-black leading-[0.95] tracking-tight text-slate-950 md:text-7xl">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link
                    ...
                    className="rounded-xl bg-orange-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-orange-600/20 transition hover:bg-orange-700"
                  >
                    Shop Trending →
                  </Link>
                  <Link
              ...
              <div className="relative z-10 flex justify-center">
                <div className="relative flex aspect-square w-full max-w-md items-stretch justify-center rounded-full bg-gradient-to-br from-orange-200 to-orange-100 shadow-2xl shadow-orange-200/50">
          ...
          <section className="mx-auto -mt-1 grid max-w-6xl grid-cols-2 gap-3 rounded-2xl bg-white p-4 shadow-xl shadow-slate-200/70 md:-mt-2 md:grid-cols-4 md:p-5">
            {[
              [icons.truck, "Free Delivery", "Free delivery on all orders"],
              [icons.shield, "Secure Payments", "100% safe & secure"],
              [icons.headset, "24/7 Support", "We're here to help"],
              [icons.return, "Easy Returns", "Hassle-free returns"],
            ].map(([icon, title, sub]) => (
              <div key={String(title)} className="flex items-center gap-3 border-slate-100 md:border-r last:md:border-r-0">
                <Icon size={28} className="shrink-0 text-orange-600">
                  {String(icon)}
                </Icon>
                <div>
          ...
          <section className="mt-12 grid gap-6 lg:grid-cols-2">
            <div className="relative overflow-hidden rounded-3xl border border-orange-100 bg-gradient-to-br from-orange-50 to-white p-8 shadow-sm">
              <div className="flex items-center gap-5">
                <div className="grid h-24 w-24 place-items-center rounded-3xl bg-white shadow-lg">
                  <Icon size={46} className="text-orange-600">
                    {icons.shirt}
                  </Icon>
                </div>
                <div>
                  <h3 className="text-2xl font-black">Upload Your Own Design</h3>
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    Get your custom design printed on T-shirts & cups — starting at ₹499!
                  </p>
                  <Link
                    to="/custom-print"
                    className="mt-4 inline-flex w-fit items-center justify-center rounded-xl bg-orange-600 px-5 py-3 text-sm font-bold text-white"
                  >
                    Start Designing <span className="ml-1">→</span>
                  </Link>
                </div>
              </div>
            </div>

            <div className="relative overflow-hidden rounded-3xl bg-orange-600 p-8 text-white shadow-xl shadow-orange-500/20">
              <div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-white/10" />
              <div className="flex items-center gap-5">
                <div className="grid h-24 w-24 place-items-center rounded-3xl bg-white/15">
                  <Icon size={48}>{icons.percent}</Icon>
                </div>
                <div>
                  <h3 className="text-2xl font-black">Use Code WELCOME10</h3>
                  <p className="mt-2 text-sm text-white/85">Get 10% off on your first order!</p>
                  <Link
                    to="/category/trending"
                    className="mt-4 inline-flex w-fit items-center justify-center rounded-xl bg-white px-5 py-3 text-sm font-bold text-orange-600"
                  >
